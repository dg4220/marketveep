...
add_action('admin_init', 'bhrc_add_multi_meta_boxes', 1);
function bhrc_add_multi_meta_boxes() {
	add_meta_box( 'repeatable-fields', 'Repeatable Fields', 'bhrc_repeatable_meta_box_display', 'representatives', 'normal', 'high');
}


function bhrc_repeatable_meta_box_display() {
	global $post;

	$repeatable_fields = get_post_meta($post->ID, 'bhrc_Room', true);
	wp_nonce_field( 'bhrc_repeatable_meta_box_nonce', 'bhrc_repeatable_meta_box_nonce' );
	?>
	...
	if ( $repeatable_fields ) :
		...
		for($i = 0; $i < count($repeatable_fields); $i+=2) {
			echo '<div><b>Test Link:</b> <a href="' . $repeatable_fields[$i+1]['bhrc_Room'] . '" target="_blank">' . $repeatable_fields[$i]['bhrc_Room'] . '</a></div>';
		...
		?>
		<tr>
			<td><input type="text" class="widefat" name="bhrc_Room[]" value="<?php if($repeatable_fields[$i]['bhrc_Room'] != '') echo esc_attr( $repeatable_fields[$i]['bhrc_Room'] ); ?>" /></td>
			<td><input type="text" class="widefat" name="bhrc_Room[]" value="<?php if($repeatable_fields[$i+1]['bhrc_Room'] != '') echo esc_attr( $repeatable_fields[$i+1]['bhrc_Room'] ); ?>" /></td>
	...
	?>
	<tr>
		<td><input type="text" class="widefat" name="bhrc_Room[]" /></td>
		<td><input type="text" class="widefat" name="bhrc_Room[]" /></td>
		<td><a class="button remove-row" href="#">Remove</a></td>
	</tr>
	<?php endif; ?>
...
add_action('save_post', 'bhrc_repeatable_meta_box_save');
function bhrc_repeatable_meta_box_save($post_id) {
	
	if ( ! isset( $_POST['bhrc_repeatable_meta_box_nonce'] ) ||
	! wp_verify_nonce( $_POST['bhrc_repeatable_meta_box_nonce'], 'bhrc_repeatable_meta_box_nonce' ) )
		return;
	...
	$old = get_post_meta($post_id, 'bhrc_Room', true);
	$new = array();
	
	//echo "<H1>Old: " . $old[0] . "</H1>";
	//echo "<H1>old_appliance: " . $old_appliance[0] . "</H1>";
	$bhrc_Room = $_POST['bhrc_Room'];
	
	//echo "<H1>Room: " . $bhrc_Room[0] . "</H1>";
	//echo "<H1>Appliance: " . $bhrc_Appliance[0] . "</H1>";
	//$bhrc_Room = count( $bhrc_Room );
	
	for ( $i = 0; $i < count( $bhrc_Room ); $i++ ) {
		if ( $bhrc_Room[$i] != '' ) :
			$new[$i]['bhrc_Room'] = stripslashes( strip_tags( $bhrc_Room[$i] ) );
		endif;
	}
	if ( !empty( $new ) && $new != $old )
		update_post_meta( $post_id, 'bhrc_Room', $new );
	elseif ( empty($new) && $old )
		delete_post_meta( $post_id, 'bhrc_Room', $old );
	...
	if ( ! wp_is_post_revision( $post_id ) ){
		...
		// unhook this function so it doesn't loop infinitely
		remove_action('save_post', 'bhrc_repeatable_meta_box_save');
